...
HlmsTextureManager::TextureLocation DSDatablock::setTexture(const String &name,
		HlmsTextureManager::TextureMapType textureType,
		DSTextureParam * paramptr) {

	HlmsManager *hlmsManager = mCreator->getHlmsManager();
	HlmsTextureManager *hlmsTextureManager = hlmsManager->getTextureManager();

	HlmsTextureManager::TextureLocation texLocation =
			hlmsTextureManager->createOrRetrieveTexture(name, textureType);
...
void DSDatablock::syncWithGPU() {
	...
	if (!(this->status == CLEAN && staticMaterial)) {
		...
		g->start(
				reinterpret_cast<float*>(this->MaterialBuffer->map(0,
		...
		for (uint i = 0; i < textureParams->size(); i++) {
			...
			if (textureParams->at(i)->isMultiTex()) {
				for (uint ii = 0; ii < textureParams->at(i)->getNumTextures();
						ii++) {
...
void DSDatablock::createTexIndexPropertys(bool caster) {
	...
	param = new DSPropertyParam(DSProperty::NumTextureParams,
			textureParams->size(),
			NULL);
	propertyParams->push_back(param);
...
//			cast = convert.str();
		...
		param = new DSPropertyParam(
				DSProperty::TextureValLoc + tex->getPostFix(),
				tex->getLocation(0)->x, NULL);
		propertyParams->push_back(param);

		param = new DSPropertyParam(
				DSProperty::TextureDefines + tex->getPostFix(), i,
				NULL);
		propertyParams->push_back(param);
...
void DSDatablock::createVec4IndexPropertys(bool Caster) {
	...
	param = new DSPropertyParam(DSProperty::NumVec4Params,
			materialParams->size(),
			NULL);
	propertyParams->push_back(param);
	...
	for (uint i = 0; i < materialParams->size(); i++) {
		...
		propertyParams->push_back(param);
		param = new DSPropertyParam(
				DSProperty::Vec4ValDefines + vec->getPostFix(), i, NULL);
		propertyParams->push_back(param);
	}
	...
	for (uint i = 0; i < materialParams->size(); i++) {
		...
		s << "\t " << vec->getTypeString(0) << " " << prefix
				<< vec->getPostFix() << vec->getArrayString(0) << ";\n";
	...
	DSPieceParam * piece = new DSPieceParam(DSProperty::MaterialVec4Params,
			value, type);
...
float*  DSDatablock::getParam(Ogre::String allocator, uint& size) {
	size=-1;
	for (std::vector<DSMaterialParam *>::iterator it = materialParams->begin();
			it != materialParams->end(); ++it) {
		if ((*it)->getParamName().compare(allocator) == 0) {
			...
			float *retv=new float[(*it)->getSize()]();
...
void DSDatablock::createMaterial() {
	...
	for (uint i = 0; i < this->propertyParams->size(); i++) {
		if (this->propertyParams->at(i)->getKey() == IdString("forward")
				&& this->propertyParams->at(i)->getValue() != 0) {
			this->dbtype = DS_DATABLOCK_FORWARD;
		}
...
bool DSDatablock::hasprop(Ogre::String str) {
	for (uint i = 0; i < this->propertyParams->size(); i++) {
		if (this->propertyParams->at(i)->getKey() == IdString(str)) {
			return true;
		}
	}
	return false;
}
bool DSDatablock::hasparam(Ogre::String str) {
	for (uint i = 0; i < this->materialParams->size(); i++) {
		if (this->materialParams->at(i)->getParamName().compare(str) == 0) {
			return true;
		}
	}
	return false;
