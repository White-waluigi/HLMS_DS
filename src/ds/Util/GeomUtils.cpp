...
void GeomUtils::createSphere(const String& strName, float radius, int nRings,
	...
	createSphere(pSphere->sharedVertexData[0], pSphereVertex->indexData[0],
			radius, nRings, nSegments, bNormals // need normals
...
void GeomUtils::createCone(const Ogre::String& strName, float radius,
	...
	createCone(pCone->sharedVertexData[0], pConeVertex->indexData[0], radius,
...
void GeomUtils::createCone(std::vector<VerticeData>& vertex,
		std::vector<Ogre::uint16> &indices, Ogre::Real radius,
		Ogre::Real height, int nVerticesInBase) {
...
//		vertex.push_back(radius * sinf(angle));
		vertex.push_back(
				VerticeData(radius * cosf(angle), height, radius * sinf(angle),
						0, 1, 0));
	}
...
void GeomUtils::createSphere(std::vector<VerticeData>& vertex,
		std::vector<Ogre::uint16>& indices, float radius, int nRings,
		int nSegments) {
