...
//************************************************************
	...
	Skydome.pedastol = 0;
	Skydome.position = Ogre::Vector3(0, -50, 0);
	Skydome.scale = Ogre::Vector3(-8000, 8000, 8000);
...
//	jollyroger.pedastol=0;
	...
	roof.materialParams2.push_back("opacity$0.3");
	roof.materialParams2.push_back("diffuse$0.3 0.3 0.8 0.5");
	//roof.blendType="transa";
	//roof.materialParams2.push_back("forward$1");
	//roof.rqID=(0x00000F00);
	...
	for (int i = 0; i < 20; i++) {
		...
		tree.materialParams1.push_back("opacity-cutoff$5");
...
//	water.materialParams.push_back("diffuse$0.1 0.1 0.9 0.7");
//	water.materialParams.push_back(
//			"normal_map$file=ghost_n.jpg,scale-uv=3.0 3.0 3.0");
//	water.materialParams.push_back("forward$1");
	water.materialParams.push_back("name$river");


	//Todo put that inside the material JSON
	water.rqID=(0x00000F00);
	...
	forward.model = "lighthouse.mesh";

	forward.materialParams.push_back(
			"normal_map$file=ghost_n.jpg,scale-uv=3.0 3.0 3.0");
//	goldstatue.materialParams.push_back("specular$1.5 0.5 0.2 150.0");
//	forward.materialParams.push_back("specular$1 1 1 600.0");
	forward.materialParams.push_back("diffuse_map$file=leafes.png");
	forward.materialParams.push_back("opacity-diffuse$1");
	forward.materialParams.push_back("opacity-sharp$0");

	forward.materialParams.push_back("opacity-cutoff$3");
	forward.materialParams.push_back("forward$1");
	//forward.blendType="transa";
	forward.rqID=(0x00000F00);
	//forward.compareFunc="apass";
	forward.materialParams.push_back("opacity$0.1");
	forward.label="reflect";
	forward.positionoffset=Ogre::Vector3(0,00,0);
	forward.scale=Ogre::Vector3(0.01,0.01,0.01);
	forward.pedastol=1;
...
void Museum::setupExhibits() {
	...
	for (int i = 0; i < exhibits.size(); i++) {
		...
		exhibits.at(i).item->setName("Exhibit_"+exhibits.at(i).model+"_"+Ogre::StringConverter::toString(i));
...
void Museum::setupLabels(Ogre::Vector3 pos, Ogre::String label) {
	...
	//item->getSubItem(1)->setDatablock(pole_db);
	pedastalItem->setName(Ogre::String("Label_")+label);
...
Ogre::Vector3 Museum::setupPedestal(Ogre::Vector3 pos) {
	...
	pedastalItem->setVisibilityFlags(0x00000001);
	pedastalItem->setCastShadows(true);
	pedastalItem->setName(Ogre::String("Pedestal_")+Ogre::StringConverter::toString(pos));
...
Museum::daystruct Museum::updateLights(float t) {
	...
	daystruct retVal;
	for (Ogre::uint i = 0; i < lights.size(); i++) {
...
void Museum::updateExhibits(float t) {
	...
	for (Ogre::uint i = 0; i < exhibits.size(); i++) {
...
//************************************
	...
	mobileLight.light->setName("flashlight");
	...
	exhibitL1.light->setName("exhibitL1");
	...
	exhibitL2.light->setName("ExhibitL2");
	...
	lhouselight.light->setName("lighthouseL");
...
Ogre::DSDatablock* Museum::getDatablock(std::vector<Ogre::String> params,
	...
	Ogre::String SBTs[] = { "transa","transc","add", "modulate", "replace" };
...
void Museum::addElement() {
	...
	Ogre::String a=Ogre::StringConverter::toString(randf());
	Ogre::String b=Ogre::StringConverter::toString(randf());
	Ogre::String c=Ogre::StringConverter::toString(randf());


	pv.push_back(std::make_pair("custom_post_material", "'diffuse =vec4(" +a+","+b+","+c+",0);'"));
	pv.push_back(std::make_pair("autoparam0", "'type=time,time=200,pulse=on'"));
	...
	std::sort(pv.begin(), pv.end());
	woodDatablock = static_cast<Ogre::DSDatablock*>(hlmsDS->createDatablock(
			Ogre::StringConverter::toString(rand()), Ogre::StringConverter::toString(rand()), *mb, *test, pv));

	Ogre::Item *woodItem = sceneManager->createItem("UFO.mesh",
			Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME,
			Ogre::SCENE_DYNAMIC);
	//item->setDatablock(pole_db);
	woodItem->setDatablock(woodDatablock);
	//item->getSubItem(1)->setDatablock(pole_db);
	woodItem->setName("RandomUFO");
	woodItem->setCastShadows(true);
	woodSn =
			sceneManager->getRootSceneNode(Ogre::SCENE_DYNAMIC)->createChildSceneNode(
					Ogre::SCENE_DYNAMIC);
	...
	woodSn->setPosition(randf()*10, 0, randf()*10);
	woodSn->attachObject(woodItem);
}
} /* namespace Demo */
