...
std::string macBundlePath()
{
    ...
    GraphicsSystem::GraphicsSystem( GameState *gameState,                                    Ogre::ColourValue backgroundColour ) :        BaseSystem( gameState ),
        mLogicSystem( 0 ),
        mSdlWindow( 0 ),
        mInputHandler( 0 ),
        mRoot( 0 ),
    ...
    void GraphicsSystem::registerHlms(void)
    {
        ...
        Ogre::Archive *archivePbs = Ogre::ArchiveManager::getSingletonPtr()->load(
                        dataFolder + "Hlms/DS/" + shaderSyntax,
                        "FileSystem", true );
        Ogre::HlmsDS *hlmsPbs = OGRE_NEW Ogre::HlmsDS( archivePbs, &library );
