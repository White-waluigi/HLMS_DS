	...
	//glow=texelFetch(worldMatBuf,(int(gl_FragCoord.x)));
		...
		for(int i=-samplerate;i<=samplerate;i++){
			for(int ii=-samplerate;ii<=samplerate;ii++){
				//samplingoffset=vec2( (i-samplerate/2 )*samplesize, (ii-samplerate/2 )* samplesize);
		...
		if(floatBitsToUint(pass.debug.z)%3u==2u){
...
@piece(SampleShadow)
		...
		vec2 Soffset=(shadowSampleTexCoord+(shadowRes.zw*samplingoffset));
        ...
        if(floatBitsToUint(pass.debug.z)%3u==0u){
